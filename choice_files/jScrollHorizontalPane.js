 ...
 * 
 * Modified by Sceneric to be compatible with jQuery > 1.3.1
 *
	...
	return this.each(
		function()
		{
			...
			if (jQuery(this).parent().is('.jScrollPaneContainer')) {
				...
				if((previousWindow != $(window).width()) && ((rightPos > $(window).width()) || (previousWindow < $(window).width())) && (settings.resize == true)) {
					...
					if($(window).width() >= _jscr_originalSizes[currentId]) {
						...
						paneWidth = $(window).width() - ($this.offset().left + _jscr_originalPos[currentId]) - 22;
					}
			...
			if (percentInView < 0.99) {
				...
				$container.append(
					jQuery('<div></div>').addClass('jScrollPaneTrack').css({'height':settings.scrollbarHeight+'px'}).append(
						jQuery('<div></div>').addClass('jScrollPaneDrag').css({'height':settings.scrollbarHeight+'px'}).append(
							jQuery('<div></div>').addClass('jScrollPaneDragLeft').css({'height':settings.scrollbarHeight+'px'}),
							jQuery('<div></div>').addClass('jScrollPaneDragRight').css({'height':settings.scrollbarHeight+'px'})
						)
					)
				);
				...
				for(inter in _jscr_intervals) { 
					...
					if(inter != 0) { 
						interObj = jQuery('<div>|</div>').addClass('jScrollIntervalTrack').css({'left':intervalTrackPos + 'px'})
